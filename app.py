...
def main():
    ...
    with st.sidebar:
        ...
        selected_mode = st.radio(
            "Choose interface",
            options=["User Booking", "Development Office"],
            index=0 if st.session_state.get("mode", "user") == "user" else 1,
            key="mode_radio"
        )

        # Update session state from radio
        if selected_mode == "User Booking":
        ...
        if st.session_state.mode == "user":
            ...
            st.info("Development Office mode active")
